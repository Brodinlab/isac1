...
maf_merge <- read.maf("Data/isac1_filtered_maf_maftools.maf", clinicalData = meta_data, verbose = F)
source("Scripts/func/tcgacompare.R")

##---------- Fig 4a
isac.mutload = tcgacompare(maf = maf_merge, cohortName = 'ISAC', logscale = TRUE,primarySite = F, capture_size = 35.8, rm_zero = FALSE, medianCol = "#7F3F98",bg_col = c("#EDF8B1", "white"),cohortFontSize = 1, axisFontSize = 1
)
...
ggplot(tmb_merge, mapping = aes(x=tumor_level2, y=total_perMB)) + geom_boxplot(fill = "#91928E", lwd = 0.05) +
  geom_point(position = "jitter", aes(color=tumor_occasion),size =2) + theme_bw() +
