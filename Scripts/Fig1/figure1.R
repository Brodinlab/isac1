...
df %>% 
  ...
  geom_line(aes(x = day, color = tumor_type_grouped), size = 1, alpha = 0.3) +
  ...
  geom_point(aes(x = 1, shape = one_year_progression_free_survival), size = 2, color = "darkgrey", show.legend = FALSE) +
  ...
  labs(x = "Years after baseline samples", y = "Patients", shape = "Outcomes", color = "Tumor type") +
  theme(panel.background = element_blank())
